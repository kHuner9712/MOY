...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!isManager(auth.profile) && workItemId) {
      const check = await auth.supabase.from("work_items").select("owner_id").eq("org_id", auth.profile.org_id).eq("id", workItemId).maybeSingle();
      if (check.error) throw new Error(check.error.message);
      const taskOwnerId = (check.data as { owner_id: string } | null)?.owner_id ?? null;
    ...
    await trackSuggestionAdoption({
      ...
      adoptionType: "viewed",
      adoptionContext: "before_followup"
    }).catch(() => null);

    return ok(result);
  } catch (error) {
    return fail(error instanceof Error ? error.message : "generate_followup_prep_failed", 500);
  }
}
