...
const requestSchema = z.object({
  ...
  meetingPurpose: z.string().min(2).max(120).optional()
});
...
export async function POST(request: Request) {
  ...
  const { customerId, opportunityId, workItemId, meetingPurpose } = parsed.data;
  ...
  try {
    ...
    await trackSuggestionAdoption({
      ...
      adoptionType: "viewed",
      adoptionContext: "before_meeting"
    }).catch(() => null);

    return ok(result);
  } catch (error) {
    return fail(error instanceof Error ? error.message : "generate_meeting_prep_failed", 500);
  }
}
