...
import { getDealRoomDetail, summarizeDealThread } from "@/services/deal-command-service";
  ...
  try {
    ...
    if (!isManager(auth.profile)) {
      ...
      if (!isParticipant && detail.room.ownerId !== auth.profile.id) {
        return fail("Sales can only summarize own/joined deal rooms", 403);
      }
    }
    ...
    const result = await summarizeDealThread({
      supabase: auth.supabase,
      orgId: auth.profile.org_id,
      dealRoomId: params.id,
    ...
    return fail(error instanceof Error ? error.message : "thread_summary_failed", 500);
  }
}
